...
using Library.ConsoleApp.Fakes;
...
var holdResult = await placeHold.Execute(
    new PlaceHoldUseCase.Command(patron.Id, book.Id));

Console.WriteLine($"予約作成: HoldId={holdResult.HoldId.Value}");
...
var assignResult = await assignCopy.Execute(
    new AssignCopyUseCase.Command(copy.Id));

Console.WriteLine($"割当完了: HoldId={assignResult!.HoldId.Value}");
Console.WriteLine($"  Hold状態: {book.Holds[0].Status}");
Console.WriteLine($"  Copy状態: {copy.Status}");
...
var checkoutResult = await checkoutBook.Execute(
